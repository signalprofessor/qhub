"""Zero-crab terrain PF driven by raw gyro and longitudinal acceleration."""
import bisect
import math
import mmap
import random
...
def _longitudinal_velocity_trace(events):
    samples = []
    for event in events:
        payload = event.get("payload", {})
        if event.get("eventType") == "navigation.imu_batch" and payload.get("sensor") == "accelerometer":
            samples.extend(payload.get("samples", []))
    samples.sort(key=lambda item: item[0])
    if len(samples) < 2:
        return [], []
    ...
    calibration = [sample for sample in samples if sample[0] <= end]
    forward_bias = sum(sample[2] for sample in calibration) / len(calibration)
    times, cumulative = [samples[0][0]], [0.0]
    velocity_change = 0.0
    previous = samples[0]
    previous_acceleration = previous[2] - forward_bias
    for sample in samples[1:]:
        dt = max(0.0, min(0.1, (sample[0] - previous[0]) / 1e9))
        acceleration = sample[2] - forward_bias
        velocity_change += .5 * (previous_acceleration + acceleration) * dt
        times.append(sample[0]); cumulative.append(velocity_change)
        previous = sample; previous_acceleration = acceleration
...
def run_gyro_accel_particle_filter(events, vertical_estimates, cache_path, ground_clearance_metres=0.8,
    ...
    first_cumulative_speed = _interpolate(acceleration_times, cumulative_speed, first_time)
    ...
    previous_cumulative_speed = first_cumulative_speed
    dr_speed = initial_speed
    ...
    with cache_path.open("rb") as source, mmap.mmap(source.fileno(), 0, access=mmap.ACCESS_READ) as data:
        for index, (event, true_east, true_north) in enumerate(usable):
            ...
            current_cumulative_speed = _interpolate(acceleration_times, cumulative_speed, payload["sensorElapsedRealtimeNanos"])
            delta_speed = current_cumulative_speed - previous_cumulative_speed
            previous_cumulative_speed = current_cumulative_speed
            heading = math.radians(initial_heading - relative_yaw)
            new_dr_speed = dr_speed + delta_speed
            de0 = .5 * (dr_speed*math.sin(previous_heading) + new_dr_speed*math.sin(heading)) * dt
            dn0 = .5 * (dr_speed*math.cos(previous_heading) + new_dr_speed*math.cos(heading)) * dt
            dr_speed = new_dr_speed
