...
The authenticated `GET /v1/session-vertical-filter?sessionId=...` endpoint runs a causal offline EKF with state `[h, hdot, b]`: phone height in metres, vertical speed, and pressure bias in hPa. GNSS measures `h`; Android `verticalAccuracyMeters` supplies its standard deviation when available, with a 20 m fallback. Pressure uses the nonlinear standard-atmosphere model `1013.25 exp(-h/8434.5) + b`, linearized at each update. The 2026-09-23 drive measured 0.0206 hPa stationary pressure noise (0.172 m), motivating pressure noise 0.024 hPa. GNSS vertical accuracy reports a median 2.01 m but misses a several-metre time-correlated bias, so the filter applies a conservative 10 m minimum GNSS standard deviation. Vertical-acceleration process noise is 0.25 m/s² and pressure-bias random-walk standard deviation is 0.1 hPa per square-root hour. These remain experimental tile-level tuning values.

For the experimental tile `6472500_535000`, GNSS altitude is transformed to the DEM vertical frame with a fixed 32.5 m offset calibrated from the 2026-09-22 drive. This is tile calibration, not an independent geoid model. The dashboard subtracts the user-entered sensor-to-ground clearance from EKF phone height before comparing it with DEM terrain height. DEM is never passed to the EKF as a measurement. This manual clearance is temporary and is intended to be replaced by laser distance.
...
## Gyro and speed-state particle-filter replay

Select **Gyro + speed state** in the dashboard to simulate GNSS loss after a known initial state. The first five stationary seconds estimate residual gyro bias after Android’s uncalibrated-sensor bias correction. Full quaternion integration of the raw 100 Hz gyro supplies relative heading; no gravity or magnetometer update is applied. The first reliable moving GNSS fix provides only initial position, speed, and course. Later GNSS positions are evaluation truth and never enter propagation or weights.

Particle state is `[E, N, speed]`. Speed follows a non-negative random walk, position uses trapezoidal integration with the common gyro heading, and barometric EKF ground height supplies the terrain likelihood. The legacy GNSS-speed/bearing model remains selectable for reference. The sensor-height control is shared by the height chart and both PF models. The initial gyro-speed model deliberately exposes the poor observability of rapidly changing vehicle speed from terrain height alone.
...
The dashboard also offers a car-development model with crab angle fixed deterministically to 0 degrees (zero initial uncertainty and zero process noise). Quaternion-integrated raw gyro supplies heading. The signed speed state is initialized once from GNSS and thereafter driven by the calibrated Android accelerometer y-channel; because the phone forward direction is negative body Y, positive sensor Y corresponds to positive vehicle acceleration. GNSS remains evaluation truth only. The particle state includes longitudinal acceleration bias with initial standard deviation 0.15 m/s² and random walk 0.003 m/s²/sqrt(s). Speed process noise is 0.3 m/s/sqrt(s). The dashboard reports posterior mean bias, particle speed range, and effective terrain-likelihood height sigma. Terrain likelihood variance is the sum of the fixed DEM-model variance and the vertical EKF height variance, preventing overconfident terrain updates when the height channel is uncertain. The unfiltered DR controls deliberately expose the raw longitudinal accelerometer bias rather than allowing velocity direction to drift away from the accurately measured gyro heading.
