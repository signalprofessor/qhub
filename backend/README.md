...
The dashboard Canvas renders all 1,000 particles, the untouched initial cloud, PF and GNSS trails, current PF mean and GNSS reference, ESS, position error, resampling, and the number of surviving initial angular sectors. Eight lineage colors are assigned before the first terrain update and survive resampling. The initial implementation is diagnostic, not yet an onboard navigation output; it deliberately exposes convergence and divergence rather than hiding them.
