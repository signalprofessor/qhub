...
def _scalar_update(x, p, residual, h, variance):
    ...
    if not math.isfinite(innovation_variance) or innovation_variance <= 0:
        return x, p, False, None
    normalized_innovation = abs(residual) / math.sqrt(innovation_variance)
    if normalized_innovation > INNOVATION_GATE_SIGMA:
        return x, p, False, normalized_innovation
    ...
    return x, updated, True, normalized_innovation
...
def filter_session(events, gnss_datum_offset_metres=0.0):
    """Return causal EKF estimates; DEM is never used as a filter measurement."""
    latest_height = None
    ...
    for event in events:
        ...
        if kind == "navigation.gnss":
            ...
            if type(value) in (int, float) and math.isfinite(value):
                latest_height = float(value) - gnss_datum_offset_metres
        ...
        measurement_type = "initialization"
        if x is None:
            ...
            if kind == "navigation.gnss" and latest_height is not None:
                ...
                x, p, accepted, normalized_innovation = _scalar_update(
                    x, p, latest_pressure - (expected + x[2]), jacobian, PRESSURE_STD_HPA ** 2)
        estimates.append({"sequence": event["sequence"],
                          ...
                          "normalizedInnovation": round(normalized_innovation, 3) if normalized_innovation is not None else None})
    return estimates
